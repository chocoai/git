...
public class AnsiSqlDialect extends Dialect {
	...
	public String forPaginate(int pageNumber, int pageSize, String select, String sqlExceptSelect) {
		String orderBy = DbSqlBuiler.GetOrderBy(sqlExceptSelect);
        if (StrKit.isBlank(orderBy))
        {
            throw new RuntimeException("非法sqlExceptSelect参数，必须包含order by 语句");
        }

        sqlExceptSelect = DbSqlBuiler.ReplaceFormatSqlOrderBy(sqlExceptSelect);

        StringBuilder sql = new StringBuilder();
        int satrt = (pageNumber - 1) * pageSize + 1;
        int end = pageNumber * pageSize;
        sql.append("select row_.* from (  ");
        sql.append(select).append(",row_number() over(").append(orderBy).append(") as rownum_ ").append(sqlExceptSelect);
        sql.append(" ) row_ where rownum_ >= ").append(satrt).append(" and rownum_ <=").append(end);
        return sql.toString();
	}
