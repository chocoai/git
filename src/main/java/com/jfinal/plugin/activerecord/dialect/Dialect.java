...
public abstract class Dialect {
    ...
    public Dialect setKeepByteAndShort(boolean keepByteAndShort) {
        ...
        /**
         * 内部的 4 个 if 判断是为了避免替换掉用户通过 setModelBuilder(...) setRecordBuilder(...)
         * 配置的自定义 builder
         */
        if (keepByteAndShort) {
     ...
     * BaseModelBuilder 生成针对 Byte、Short 类型的获取方法：
     ...
     * 配置自定义 ModelBuilder
     *
     * 通过继承扩展 ModelBuilder 可以对 JDBC 到 java 数据类型进行定制化转换 不同数据库从 JDBC 到 java
     * 数据类型的映射关系有所不同
     *
    ...
    /**
     * 覆盖此方法，可以对 JDBC 到 java 数据类型进行定制化转换 不同数据库从 JDBC 到 java 数据类型的映射关系有所不同
     *
     * 此外，还可以通过改变 RecordBuilder.buildLabelNamesAndTypes() 方法逻辑，实现下划线字段名转驼峰变量名的功能
     */
    ...
    /**
     * 用于获取 Model.save() 以后自动生成的主键值，可通过覆盖此方法实现更精细的控制 目前只有
    ...
    /**
     * mysql 数据库的 bigint unsigned 对应的 java 类型为 BigInteger 但是 rs.getObject(1)
     * 返回值为 Long 型，造成 model.save() 以后 model.getId() 时的类型转换异常
     */
     ...
     * PostgreSql 使用的 "\"" 字符 不满足以上两个条件之一的 forDbXxx 系列方法也可以使用
     * trimPrimaryKeys(...) 方法让 sql 更加美观，但不是必须
     *
